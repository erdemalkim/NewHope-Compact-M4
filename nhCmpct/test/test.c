...
static int test_keys(void)
{
  ...
  for(i=0; i<NTESTS; i++)
  {
    ...
    hal_send_rstr("DONE key pair generation!");

    //Bob derives a secret key and creates a response
    crypto_kem_enc(sendb+8, key_b+8, pk+8);
    hal_send_rstr("DONE encapsulation!");

    //Alice uses Bobs response to get her secret key
    crypto_kem_dec(key_a+8, sendb+8, sk_a+8);
    hal_send_rstr("DONE decapsulation!");
    ...
    {
      hal_send_rstr("ERROR canary overwritten\n");
    }
    else
    {
...
static int test_invalid_sk_a(void)
{
  ...
  for(i=0; i<NTESTS; i++)
  {
    ...
    {
      hal_send_rstr("ERROR invalid sk_a\n");
    }
    else
    {
      hal_send_rstr("OK invalid sk_a\n");
    }
  }
